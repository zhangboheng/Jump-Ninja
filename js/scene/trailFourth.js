...
export default class TrailFourth {
  constructor(game) {
    this.game = game;
    ...
    this.boardsLeft = [{
      ...
      x: this.canvas.width / 2 - 125 * scaleX,
      y: this.canvas.height - 480 * scaleY,
      width: 12 * scaleX,
      height: 60 * scaleY,
      type: 'wood',
      smooth: 0
  ...
  drawMessageBox() {
    if(this.displayMessageTime > 0) {
      showBoxMessage(this.context, '试炼 - 04', this.canvas.width / 2, this.canvas.height / 2, '#f5d659', 'black', 20 * scaleX);
      setTimeout(() => {
