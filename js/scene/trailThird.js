...
export default class TrailThird {
  ...
  drawGoal() {
    ...
    if (this.endImage.complete) {
      this.context.drawImage(this.endImage, this.canvas.width / 2 - 56, this.canvas.height - 800 - this.endImage.height + groundY, this.endImage.width, this.endImage.height);
    }
  }
  // 绘制返回按钮
  drawBack() {
  ...
  drawMessageBox() {
    ...
    if(this.displayMessageTime > 0) {
      showBoxMessage(this.context, '试炼 - 03', this.canvas.width / 2, this.canvas.height / 2);
      setTimeout(() => {
