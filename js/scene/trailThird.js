...
export default class TrailThird {
  constructor(game) {
    this.game = game;
  ...
  drawGoal() {
    if (this.endImage.complete) {
      this.context.drawImage(this.endImage, this.canvas.width / 2 - 56 * scaleX, this.canvas.height - 800 * scaleY - this.endImage.height * scaleY + this.ninja.groundY, this.endImage.width * scaleX, this.endImage.height * scaleY);
    }
  }
  // 绘制返回按钮
  drawBack() {
  ...
  drawMessageBox() {
    if(this.displayMessageTime > 0) {
      showBoxMessage(this.context, '试炼 - 03', this.canvas.width / 2, this.canvas.height / 2, '#f5d659', 'black', 20 * scaleX);
      setTimeout(() => {
