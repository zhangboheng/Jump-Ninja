...
export default class TrailFirst {
  constructor(game) {
    this.game = game;
  ...
  drawGoal() {
    if (this.endImage.complete) {
      this.context.drawImage(this.endImage, this.canvas.width / 2 - 88 * scaleX, this.canvas.height - 550 * scaleY - this.endImage.height * scaleY + this.ninja.groundY, this.endImage.width * scaleX, this.endImage.height * scaleY);
    }
  }
  // 绘制返回按钮
  drawBack() {
  ...
  drawMessageBox() {
    if(this.displayMessageTime > 0) {
      showBoxMessage(this.context, '试炼 - 01', this.canvas.width / 2, this.canvas.height / 2, '#f5d659', 'black', 20 * scaleX);
      setTimeout(() => {
