...
export default class Trailseventh {
  constructor(game) {
    this.game = game;
    ...
    this.boardsLeft = [{
      ...
      x: this.canvas.width / 2 - 70 * scaleX,
      y: this.canvas.height - 800 * scaleY,
      width: 12 * scaleX,
      height: 250 * scaleY,
      type: 'glass',
  ...
  drawGoal() {
    if (this.endImage.complete) {
      this.context.drawImage(this.endImage, this.canvas.width / 2 - 100 * scaleX - 16 * scaleY, this.canvas.height - 650 * scaleY - 64 * scaleY + this.ninja.groundY, 64 * scaleY, 64 * scaleY);
    }
  }
  // 绘制返回按钮
  drawBack() {
  ...
  drawMessageBox() {
    if (this.displayMessageTime > 0) {
      showBoxMessage(this.context, '试炼 - 07', this.canvas.width / 2, this.canvas.height / 2, '#f5d659', 'black', 20 * scaleX);
      setTimeout(() => {
  ...
  drawCycleLogo() {
    if (this.cycleImage.complete && this.showCycle) {
      this.context.save();
      this.context.translate(this.canvas.width / 2, this.canvas.height - 700 * scaleY + this.ninja.groundY);
