...
export default class TrailSixth {
  ...
  drawGoal() {
    ...
    if (this.boards[6].clockLimit >= 2) {
      if (this.endImage.complete) {
        this.context.drawImage(this.endImage, this.canvas.width / 2 + 90, this.canvas.height - 1600 - this.endImage.height + groundY, this.endImage.width, this.endImage.height);
      }
    }
  }
  // 绘制返回按钮
  ...
  drawMessageBox() {
    ...
    if(this.displayMessageTime > 0) {
      showBoxMessage(this.context, '试炼 - 06', this.canvas.width / 2, this.canvas.height / 2);
      setTimeout(() => {
  ...
  updateNinja() {
    ...
    if (this.ninja.toRight) {
      ...
      // 是否抓住木板
      for (const board of this.boardsRight.filter(item=>item.clockLimit >= 2)) {
        if (
