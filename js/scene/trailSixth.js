...
export default class Trailsixth {
  constructor(game) {
    this.game = game;
    ...
    this.boardsLeft = [{
      ...
      y: this.canvas.height - 550 * scaleY,
      width: 6 * scaleX,
      height: 120 * scaleY,
      type: 'ice',
      smooth: 8,
      ...
      x: this.canvas.width / 2 - 100 * scaleX,
      y: this.canvas.height - 1400 * scaleY,
      width: 20 * scaleX,
      height: 100 * scaleY,
      type: 'wood',
      smooth: 1,
    ...
    this.boardsRight = [{
      ...
      y: this.canvas.height - 400 * scaleY,
      width: 10 * scaleX,
      height: 100 * scaleY,
      type: 'metal',
      smooth: 0.01,
  ...
  // 绘制终点图标
  drawGoal() {
    if (this.endImage.complete && this.boards[6].clockLimit >= 2) {
      this.context.drawImage(this.endImage, this.canvas.width / 2 + 86 * scaleX, this.canvas.height - 1600 * scaleY - this.endImage.height * scaleY + this.ninja.groundY, this.endImage.width * scaleX, this.endImage.height * scaleY);
    }
  }
  // 绘制返回按钮
  drawBack() {
  ...
  drawMessageBox() {
    if(this.displayMessageTime > 0) {
      showBoxMessage(this.context, '试炼 - 06', this.canvas.width / 2, this.canvas.height / 2, '#f5d659', 'black', 20 * scaleX);
      setTimeout(() => {
  ...
  drawClock() {
    if (this.showClock) {
      if (this.clockImage.complete) {
        this.context.save();
        this.context.translate(this.canvas.width / 2 - this.clockImage.width * scaleY / 2 + 20 * scaleX, this.canvas.height - 1350 * scaleY + this.ninja.groundY);
  ...
  // 绘制木板
  drawBoard() {
    for (const board of this.boards.filter(item=>item.clockLimit >= 2)) {
        if (board.type == 'wood') {
