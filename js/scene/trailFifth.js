...
export default class Scene1 {
  ...
  drawGoal() {
    ...
    if (this.boards[4].showLimit >= 8) {
      if (this.endImage.complete) {
        this.context.drawImage(this.endImage, this.canvas.width / 2 + 86, this.canvas.height - 1200 - this.endImage.height + groundY, this.endImage.width, this.endImage.height);
      }
    }
  }
  // 绘制返回按钮
  ...
  drawMessageBox() {
    ...
    if(this.displayMessageTime > 0) {
      showBoxMessage(this.context, '试炼 - 05', this.canvas.width / 2, this.canvas.height / 2);
      setTimeout(() => {
  ...
  drawCycleLogo() {
    ...
    if (this.showCycle) {
      if (this.cycleImage.complete) {
        this.context.drawImage(this.cycleImage, this.canvas.width / 2 - this.cycleImage.width / 2, this.canvas.height - 1000 + groundY, this.cycleImage.width, this.cycleImage.height);
      }
    }
  }
  // 绘制忍者
  ...
  updateNinja() {
    ...
    if (this.ninja.toRight) {
      ...
      // 是否抓住木板
      for (const board of this.boardsRight.filter(item=>item.showLimit >= 8)) {
        if (
  ...
  drawBoard() {
    ...
    for (const board of this.boards.filter(item=>item.showLimit >= 8)) {
      if (board.type == 'wood') {
